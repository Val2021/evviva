...
class EmbeddingService:
    ...
    def embed_document(self, text: str):
        dense = list(self.dense_model.passage_embed([text]))[0].tolist()
        sparse = list(self.sparse_model.passage_embed([text]))[0].as_object()
        colbert = list(self.colbert_model.passage_embed([text]))[0].tolist()
